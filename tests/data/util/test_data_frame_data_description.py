...
def test_get_raw_data_option():
    dd = DataFrameDataDescription(
        TEST_DF_OPTION,
        "value",
        value_to_tensor=lambda x: -x,
    )
    for sample_id in TEST_DF.index.levels[0]:
        options = dd.get_loading_options(sample_id)
...
def test_get_raw_data():
    ...
    for sample_id in TEST_DF.index.levels[0]:
        val = dd.get_raw_data(sample_id, None)
        assert val == -TEST_DF.loc[sample_id][0]
